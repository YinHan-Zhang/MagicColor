...
class ImagePairEditPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if self.reference_unet:
            refnet_encoder_hidden_states = img2embeds(ref1, self.refnet_image_enc)
        else:
            refnet_encoder_hidden_states = None
        if self.controlnet:
            controlnet_encoder_hidden_states = img2embeds(ref1, self.controlnet_image_enc)
        else:
        ...
        # --------------- Image Processing ------------------------
        # print(f"Image Processing Shape: {ref1.size}") # 512,512
        # Resize image
        if processing_res > 0:
            def resize_img(img):
        ...
        # print(f"Image Normalize Shape: {ref1.shape}") # 3,512,512
        ...
        return ImagePairEditPipelineOutput(
            img_np=img_pred_np,
    ...
    def feature_transfer(self, src_ft, trg_ft):
        ...
        trg_pts = grid_trg.reshape(-1, 2)[indices].cpu().numpy()  # 源图像在目标图像上对应的点
        ...
        for i, (src_pt, trg_pt) in enumerate(zip(src_pts, trg_pts)):
            ...
            trans_ft[:, :, trg_y, trg_x] = src_ft[:, :, src_y, src_x] # 直接替换
    ...
    def single_infer(
        ...
        # bg_latents = self.encode_RGB(bg, generator=generator) 
        ...
        for i, t in iterable:
            ...
            # refnet_bg_input = bg_latents
            controlnet_inputs = (noisy_edit2_latents, edge2) # add sketch to controlnet
            unet_input = torch.cat([noisy_edit2_latents], dim=1)
            
            # 1. add ref to ref unet
            if self.reference_unet and i == 0: 
            ...
            if i == 0:
                # update reference attn at first step
            ...
            # 7. compute the previous noisy sample x_t -> x_t-1
